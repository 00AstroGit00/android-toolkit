# Support

Thank you for using the Android Toolkit! Here's how to get help.

## Documentation

Start with the project documentation — most questions are answered there:

| Document | Purpose |
|----------|---------|
| [README.md](README.md) | Overview, installation, quick start |
| [CONTRIBUTING.md](CONTRIBUTING.md) | How to contribute, coding standards |
| [SECURITY.md](SECURITY.md) | Security policies and vulnerability reporting |
| [LTS-POLICY.md](LTS-POLICY.md) | Long-term support schedule |
| [docs/RELEASE_PROCESS.md](docs/RELEASE_PROCESS.md) | Release cadence and backport policy |
| [PLUGIN_API.md](PLUGIN_API.md) | Plugin SDK reference |
| [ROADMAP_V5.md](ROADMAP_V5.md) | Future development plans |

## Getting Help

1. **Check the docs first** — see the table above.
2. **Search existing issues** — your question may have been answered already.
3. **Open an issue** if you can't find an answer:
   - [Bug Report](https://github.com/YOUR_USERNAME/android-toolkit/issues/new?template=01-bug-report.md)
   - [Documentation Issue](https://github.com/YOUR_USERNAME/android-toolkit/issues/new?template=06-documentation-issue.md)
   - [Plugin Issue](https://github.com/YOUR_USERNAME/android-toolkit/issues/new?template=05-plugin-issue.md)

## Community

- **Discussions**: GitHub Discussions (if enabled)
- **Issues**: [GitHub Issues](https://github.com/YOUR_USERNAME/android-toolkit/issues)

## Commercial Support

For enterprise or commercial support inquiries, please open a discussion or
contact the maintainers through GitHub.

## Reporting a Vulnerability

**Do not open public issues for security vulnerabilities.**  
Follow the disclosure process in [SECURITY.md](SECURITY.md).
